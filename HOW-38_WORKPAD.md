...
- Plan
  ...
  - [completed] Fix stale success notification payloads so Telegram completion replies report the just-verified item instead of an older cart snapshot item.
...
- Validation Checklist
  ...
  - [x] `POSTGRES_DSN='postgresql://postgres:postgres@localhost:5432/coupang_cart_agent' COUPANG_BROWSER_LAUNCH_MODE=browser_use COUPANG_CHROME_USER_DATA_DIR="$HOME/Library/Application Support/Google/Chrome" COUPANG_CHROME_PROFILE_DIRECTORY='Default' COUPANG_BROWSER_HEADLESS=false uv run python -m coupang_cart_agent integration-live-telegram-once --timeout 30 --intake-db-path .artifacts/how38_telegram_intake_turn3.sqlite3 --skip-error-response`
  - [x] `POSTGRES_DSN='postgresql://postgres:postgres@localhost:5432/coupang_cart_agent' COUPANG_BROWSER_LAUNCH_MODE=browser_use COUPANG_CHROME_USER_DATA_DIR="$HOME/Library/Application Support/Google/Chrome" COUPANG_CHROME_PROFILE_DIRECTORY='Default' COUPANG_BROWSER_HEADLESS=false uv run python -m coupang_cart_agent integration-live-telegram-worker --timeout 1 --sleep-seconds 0 --max-cycles 1 --intake-db-path .artifacts/how38_telegram_intake_turn3.sqlite3 --skip-error-response --worker-name how38-turn3-worker`
  - [x] `uv run python -m unittest tests.test_notifications`
...
- Notes / Blockers
  ...
  - March 18 live validation restored the fixture-free default path:
    ...
    - A real human-originated Telegram Web message produced fixture-free intake evidence: `integration-live-telegram-once --timeout 30 --intake-db-path .artifacts/how38_telegram_intake_turn3.sqlite3 --skip-error-response` captured `telegram-update-286968924` with raw text `양파 담아줘`, created a `candidate_source_mode=live_browser` proposal, and persisted thread status `awaiting_user_confirmation`.
    - A same-session Telegram confirmation then closed end to end: `integration-live-telegram-worker --timeout 1 --sleep-seconds 0 --max-cycles 1 --intake-db-path .artifacts/how38_telegram_intake_turn3.sqlite3 --skip-error-response --worker-name how38-turn3-worker` processed `telegram-update-286968924` and `telegram-update-286968925` in order, drained pending intake rows to zero, advanced the worker cursor to `286968926`, and persisted `telegram-update-286968925` as `success=true`, `conversation_status=completed`, `user_decision=confirm`.
    - During that replay, the workflow exposed a real wrong-item completion bug: the verified cart result for `telegram-update-286968925` showed onion add-to-cart success, but the success notification payload and Telegram chat message still reported an older cereal cart snapshot item.
    - The stale-success bug was fixed in `build_success_notification_payload` and verified both in tests and in a second real Telegram replay. Fresh Telegram updates `286968926` (`양파 담아줘`) and `286968927` (`ㅇㅇ 담아줘`) were processed by `integration-live-telegram-worker --offset 286968926 --timeout 1 --sleep-seconds 0 --max-cycles 1 --intake-db-path .artifacts/how38_telegram_intake_turn3_fix.sqlite3 --skip-error-response --worker-name how38-turn3-fix-worker`; Telegram Web then showed the corrected completion reply `장바구니 담기를 완료했습니다. - 친환경 깐양파, 300g, 1개 / 2,930원 / 1개`.
