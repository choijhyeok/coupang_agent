...
- Notes / Blockers
  ...
  - Follow-up revalidation at `2026-03-18 11:17:32 KST (+0900)` reached the same conclusion:
    - `curl https://api.telegram.org/bot$TELEGRAM_BOT_TOKEN/getUpdates?timeout=1` returned `{"ok":true,"result":[]}` before and after a fresh live polling attempt.
    - `curl -X POST https://api.telegram.org/bot$TELEGRAM_BOT_TOKEN/sendMessage -d chat_id=8201584878 --data-urlencode text='HOW-38 validation is waiting on one new shopping message for live Telegram intake. Reply with a request like 양파 담아줘 if available.'` succeeded with outbound `message_id=128`, confirming bot reachability to the target chat.
    - `POSTGRES_DSN='postgresql://postgres:postgres@localhost:5432/coupang_cart_agent' COUPANG_BROWSER_LAUNCH_MODE=browser_use COUPANG_CHROME_USER_DATA_DIR="$HOME/Library/Application Support/Google/Chrome" COUPANG_CHROME_PROFILE_DIRECTORY='Default' COUPANG_BROWSER_HEADLESS=false uv run python -m coupang_cart_agent integration-live-telegram-once --timeout 180 --intake-db-path .artifacts/how38_telegram_intake.sqlite3 --skip-error-response` still ended with `{"mode":"live-telegram-once","intake_results":[],"message":"No parseable Telegram requests were captured."}`.
    - The fresh intake DB remained empty: `telegram_inbound_messages` had zero rows, so no inbound Telegram update reached the app during the polling window.
